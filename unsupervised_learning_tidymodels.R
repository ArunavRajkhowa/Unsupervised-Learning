...
sd=apply(cars,2,sd)
# experiment with other measures of central tendency
# and variability and see how the grouping changes

# Agglomerative[Hierarchical] Clustering----------------------

cars.std=data.frame(scale(cars,center=medians,scale=sd)) #standardizing the data

cars.dist = dist(cars.std) # makes a half matrix

cars.hclust = hclust(cars.dist,method = "complete")

fviz_dend(cars.hclust, main = "A beautiful Dendogram",k=3)
...
diss=daisy(cars.std) #makes distance matrix like dist

sk=silhouette(groups.3,diss) # to check the quality of clustering
plot(sk)
...
# alcohol, ph , sulphates ->business call to choose which features

wq=wq %>% select(alcohol,pH,sulphates)

wq.std=scale(wq) #deafult arguments : is subtraacting mean and dividing by std

kclusts =
  tibble(k = 1:10) %>% #value of k range
  mutate(
